...
pbc_param_t par;
pairing_t pairing;
element_t G1, G2, GT, Zp, m, m_p, r_1, r_2, r_1_p, r_2_p, h, tk_1, tk_2;
...
void init_type(std::string &param) {
    ...
    element_init_same_as(r_1, G2);
    element_init_same_as(tk_2, G1);
    element_init_same_as(r_1_p, G2);
    element_init_same_as(r_2, G1);
    element_init_same_as(r_2_p, G1);
    element_init_same_as(tk_1, G2);
...
void IB_CH_nonRO_test() {
    I = new ElementList(n, -1, Zp, 1, 1);

    IB_CH_nonRO *test = new IB_CH_nonRO(&G1, &G2, &Zp, &GT, rev_G1G2);

    if(out_file) {
        test->Setup(n);
        fprintf(out, "public_key: %d B, secret_key: %d B, hash_value: %d B, random_value: %d B\n", test->public_key_size(), CountSize(tk_1) + CountSize(tk_2), CountSize(h), CountSize(r_1) + CountSize(r_2));
    }
    ...
    for(int _ = 0;_ < turns_collision;_++) {
        element_random(m_p);
        ts = std::chrono::high_resolution_clock::now();
        test->Collision(&tk_1, &tk_2, &h, &m, &m_p, &r_1, &r_2, &r_1_p, &r_2_p);
        te = std::chrono::high_resolution_clock::now();
        // printf("%d\n", test->Verify(I, &h, &m_p, &r_1_p, &r_2_p));
...
int main(int argc, char *argv[]) { // curve, scheme, turns, T;
    if(argc < 5) {
        printf("usage: %s [a|e|i|f|d224] {total_turns} [setup|hash|keygen|collision|all] [0|1] {identity length}\n", argv[0]);
        return 0;
    }
    turns = atoi(argv[2]);
    n = atoi(argv[5]);
