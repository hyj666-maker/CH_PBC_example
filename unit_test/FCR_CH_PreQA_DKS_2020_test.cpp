...
element_t h;  // hash
element_t e1,e2,s11,s12,s2;  // π
element_t e1_p,e2_p,s11_p,s12_p,s2_p;  // π_p
...
void init_type(std::string &param) {
    ...
    element_init_same_as(s11, Zp);
    element_init_same_as(s12, Zp);
    element_init_same_as(s2, Zp);

    element_init_same_as(e1_p, Zp);
    element_init_same_as(e2_p, Zp);
    element_init_same_as(s11_p, Zp);
    element_init_same_as(s12_p, Zp);
...
void FCR_CH_PreQA_DKS_2020_test() {
    printf("FCR_CH_PreQA_DKS_2020_test begin\n");
    ...
    FCR_CH_PreQA_DKS_2020 *test = new FCR_CH_PreQA_DKS_2020(&G1, &G2, &Zp, &GT);
    ...
    for(int _ = 0;_ < turns_h;_++) {
        ts = std::chrono::high_resolution_clock::now();
        test->Hash(&m,&y, &h, &e1,&e2,&s11,&s12,&s2);
        if(test->Check(&y, &m, &h, &e1, &e2, &s11, &s12, &s2)){
    ...
    for(int _ = 0;_ < turns_f;_++) {
        ts = std::chrono::high_resolution_clock::now();
        test->Forge(&m, &m_p, &e1, &e2, &s11, &s12, &s2, &h, &e1_p, &e2_p, &s11_p, &s12_p, &s2_p);
        te = std::chrono::high_resolution_clock::now();
...
int main(int argc, char *argv[]) { // curve, scheme, turns, T;
    ...
    out = fopen("tmp_FCR_CH_PreQA_DKS_2020.txt", "w");
    fflush(out);
