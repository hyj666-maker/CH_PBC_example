...
int turns = 0, turns_setup = 1, turns_keygen = 1, turns_hash = 1, turns_collision = 1, n = -1, rev_G1G2;

const bool out_file = true, visiable = false;

pbc_param_t par;
pairing_t pairing;
element_t G1, G2, GT, Zp, ID, m, m_p, h, r_1, r_2, r_3, r_1_p, r_2_p, r_3_p, td_1, td_2, L;
...
void init_type(std::string &param) {
    ...
    element_init_same_as(r_3, G1);
    element_init_same_as(r_3_p, G1);
    element_init_same_as(td_2, G2);
    element_init_same_as(r_2, G2);
    element_init_same_as(r_2_p, G2);
    element_init_same_as(h, GT);
    element_init_same_as(ID, Zp);
    element_init_same_as(m, Zp);
    element_init_same_as(m_p, Zp);
    element_init_same_as(td_1, Zp);
    element_init_same_as(r_1, Zp);
...
void IB_CH_Our_test() {
    Our_IB_CH *test = new Our_IB_CH(&G1, &G2, &Zp, &GT, rev_G1G2);
    ...
    for(int _ = 0;_ < turns_collision;_++) {
        element_random(m_p);
        ts = std::chrono::high_resolution_clock::now();
        test->Collision(&td_1, &td_2, &m, &m_p, &r_1, &r_2, &r_1_p, &r_2_p);
        te = std::chrono::high_resolution_clock::now();
...
void IB_CH_KEF_test() {
    ...
    for(int _ = 0;_ < turns_hash;_++) {
        element_random(m);
        ts = std::chrono::high_resolution_clock::now();
        test->Hash(&h, &m, &ID, &L, &r_1, &r_2, &r_3);
        te = std::chrono::high_resolution_clock::now();
        OutTime("hash", _, time_cast(te, ts));
    }
    ...
    for(int _ = 0;_ < turns_collision;_++) {
        element_random(m_p);
        ts = std::chrono::high_resolution_clock::now();
        test->Collision(&ID, &td_1, &td_2, &L, &m, &m_p, &r_1, &r_2, &r_3, &r_1_p, &r_2_p, &r_3_p);
        te = std::chrono::high_resolution_clock::now();
        // printf("%d\n", test->Verify(&h, &m_p, &ID, &L, &r_1_p, &r_2_p, &r_3_p));
        OutTime("collision", _, time_cast(te, ts));
    }
...
int main(int argc, char *argv[]) { // curve, scheme, turns, T;
    if(argc < 5) {
        printf("usage: %s [a|e|i|f|d224] {total_turns} [setup|hash|keygen|collision|all] [0|1] [base|KEF]\n", argv[0]);
        return 0;
    }
    turns = atoi(argv[2]);
    ...
    out = fopen("tmp.txt", "w");
    fflush(out);
    if(strcmp(argv[5], "base") == 0) IB_CH_Our_test();
    else if(strcmp(argv[5], "KEF") == 0) IB_CH_KEF_test();
