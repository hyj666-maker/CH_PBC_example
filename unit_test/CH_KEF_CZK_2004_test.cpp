...
void CH_KEF_CZK_2004_test() {
    printf("CH_KEF_CZK_2004_test begin\n");
    ...
    CH_KEF_CZK_2004 *test = new CH_KEF_CZK_2004(&G1, &G2, &Zp, &GT);
    ...
    for(int _ = 0;_ < turns_f;_++) {
        ts = std::chrono::high_resolution_clock::now();
        test->Forge(&h, &m, &r1, &r2, &m_p, &I, &r1_p, &r2_p);
        te = std::chrono::high_resolution_clock::now();
...
int main(int argc, char *argv[]) { // curve, scheme, turns, T;
    ...
    out = fopen("tmp_CH_KEF_CZK_2004.txt", "w");
    fflush(out);
