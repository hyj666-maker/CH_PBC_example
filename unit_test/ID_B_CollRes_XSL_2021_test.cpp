...
element_t msk;
element_t tk1,tk2;  // trapdoor key tk(tk1, tk2)
element_t r1,r2;  // r1,r2 ∈ G
element_t h;  // hash
...
void init_type(std::string &param) {
    ...
    element_init_same_as(msk, G1); 
    element_init_same_as(tk1, G1); 
    element_init_same_as(tk2, G1); 
    element_init_same_as(r1, G1);  
    element_init_same_as(r2, G1); 
    element_init_same_as(h, GT); 
    element_init_same_as(m, Zp);  
    element_init_same_as(m_p, Zp); 
    element_init_same_as(I, Zp); 
    element_init_same_as(r1_p, G1);  
    element_init_same_as(r2_p, G1); 
}
...
void ID_B_CollRes_XSL_2021_test() {
    printf("ID_B_CollRes_XSL_2021_test begin\n");
    ...
    ID_B_CollRes_XSL_2021 *test = new ID_B_CollRes_XSL_2021(&G1, &G2, &Zp, &GT);
    
    printf("——————————PG() start——————————\n");
    for(int _ = 0;_ < turns_pg;_++) {
        ts = std::chrono::high_resolution_clock::now();
        test->PG(m_bit_length, &msk);
    ...
    for(int _ = 0;_ < turns_f;_++) {
        ts = std::chrono::high_resolution_clock::now();
        test->Forge(&tk1, &tk2, &h, &m, &r1, &r2, &m_p,&r1_p,&r2_p);
        te = std::chrono::high_resolution_clock::now();
...
int main(int argc, char *argv[]) { // curve, scheme, turns, T;
    ...
    out = fopen("tmp_ID_B_CollRes_XSL_2021.txt", "w");
    fflush(out);
